...
Integration tests execute `dist/index.js`, so **run `npm run build` before `npm test`** after changing `src/`. CI fails if `dist/` is stale relative to `src/` (Linux check job).
...
- `src/session/` — session dir setup (symlink-rejected, 0700) and login **bound to the PAT identity**: a SHA-256 fingerprint of the PAT is stored in the session dir; a valid session with a different/unknown fingerprint is logged out and replaced. The session dir is saved to action state before authentication so the post step can remove it after failures.
...
`tests/fixtures/mock-pass-cli.mjs` is the test double for the real `pass-cli` (real one needs Proton Pass Plus+). `tests/fixtures/materialize-mock.mjs` supplies the shared POSIX/Windows shim writer used by the local helper and smoke installer. The mock keeps synthetic authentication state in each test session directory. `tests/helpers/run-action.ts` spawns the built bundles with `INPUT_*` env vars and parses temp `$GITHUB_ENV`/`$GITHUB_OUTPUT`/`$GITHUB_STATE` files (heredoc-aware: `tests/helpers/file-commands.ts`).
...
- `runActionLifecycle` runs main and post with the actual saved state and exported environment. Its tests inspect session removal before test teardown.
- Test helpers discard inherited `PROTON_PASS_*` and `MOCK_PASS_CLI_*` variables. Supply test inputs explicitly; session and command-file paths belong to the test.
