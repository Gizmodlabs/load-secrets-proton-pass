...
for (const [failure, responses] of [
  ['login', { info: [1], login: 1 }],
  ['session probe', { info: [1, 1], login: 0 }],
] as const) {
  test(`saves the session directory for post cleanup when ${failure} fails`, async () => {
    const { runner } = fakeRunner({ ...responses, info: [...responses.info] })
    await assert.rejects(establishSession(PAT, runner))
    assert.deepEqual(readFileCommands(process.env.GITHUB_STATE ?? ''), { 'session-dir': sessionDir })
  })
}

test('saves cleanup state before invoking pass-cli', async () => {
  const { runner } = fakeRunner({ info: [1, 0] })
  const checkingRunner: CliRunner = async (args, extraEnv) => {
    assert.deepEqual(readFileCommands(process.env.GITHUB_STATE ?? ''), { 'session-dir': sessionDir })
    return runner(args, extraEnv)
  }
  await establishSession(PAT, checkingRunner)
})

test('saves cleanup state before exporting the session environment', async () => {
  process.env.GITHUB_ENV = join(sessionDir, 'missing', 'env')
  const { runner, calls } = fakeRunner({ info: [1, 0] })
  await assert.rejects(establishSession(PAT, runner), /Missing file at path/)
  assert.deepEqual(readFileCommands(process.env.GITHUB_STATE ?? ''), { 'session-dir': sessionDir })
