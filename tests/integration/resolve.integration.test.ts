...
test('T5: resolves multiple secrets and leaves non-pass:// vars alone', async () => {
  ...
  assert.equal(result.exitCode, 0)
  assert.equal(result.env['SECRET_A'], 'mock-real-password')
  assert.equal(result.env['SECRET_B'], 'mock@example.com')
...
test('T7: field glob expands pass://V/item/* into per-field env vars', async () => {
  ...
  assert.equal(result.exitCode, 0)
  assert.equal(result.env['DB_HOST'], 'db.example.com')
  assert.equal(result.env['DB_PORT'], '5432')
  assert.equal(result.env['DB_PASSWORD'], 'hunter2')
})
...
test('T11: mixed-case/space/dash field names map to clean suffixes', async () => {
  ...
  assert.equal(result.exitCode, 0)
  assert.equal(result.env['CFG_API_KEY'], 'sanitize-apikey-value')
  assert.equal(result.env['CFG_DATABASE_NAME'], 'sanitize-dbname-value')
})
...
test('T13: glob and single URI resolve in one run', async () => {
  ...
  assert.equal(result.env['PASSWORD'], 'mock-real-password')
})
...
test('T19: resolved-keys is sorted names only; per-var outputs exist', async () => {
  ...
  assert.equal(result.output['ZEBRA_KEY'], 'mock-real-password')
})
